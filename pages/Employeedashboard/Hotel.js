...
export default function Hotel() {
  ...
  const router = useRouter();
    ...
    const handleAddClick = () => {
      router.push(`/Employeedashboard/AddHotel`);
    };
   
    const handleEditClick = (packageId) => {
      router.push(`/Employeedashboard/HoteleditPackage?id=${packageId}`);
    };
  const handleDeleteClick = async (packageId) => {
    const isConfirmed = window.confirm("Are you sure you want to delete this package?");
    if (isConfirmed) {
      try {
        await axios.delete(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/employee/deletehotel/${packageId}`, { withCredentials: true });
